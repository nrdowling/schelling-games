...
ma.responses <- responses %>% 
    filter(uc_status %ilike% "MAPSS")

# Quick labels for in-text reference to specific values

str_times <- as.character(filter(ma.responses, !is.na(str_when))$str_when)
...
str_meetup <- data.table(clean_meetup(ma.responses, "str_where", "str_error"))
...
ggplot(filter(ma.responses, !is.na(str_when)), aes(x=str_when)) + 
    ...
    labs(x = "TIME OF DAY",
         ...
         subtitle = paste0(nrow(filter(ma.responses,!is.na(str_when))), " total responses"))
...
student_meetup <- data.table(clean_meetup(ma.responses, "student_where", "student_error"))
...
friend_meetup <- data.table(clean_meetup(ma.responses, "friend_where", "friend_error"))
