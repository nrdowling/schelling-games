...
<span style="color:blue">**WHERE** do you go to meet them?</span>
...
plot_meetup(str_meetup, "str_where_clean", title="WHERE do you meet a STRANGER in Chicago?")
...
kbl_responses(str_meetup, "str_where_clean") %>% kable_styling()
```
...
<span style="color:blue">**WHAT TIME** do you go to meet them?</span>
...
# Show kable of all (cleaned) responses with raw counts and percent
kbl_responses(student_meetup, "student_where_clean") %>% kable_styling()
...
filter(student_meetup, student_where_clean %in% student_others) %>% 
    ...
    pull(student_where) %>% 
    levels() %>% 
    kable(col.names = c("Other responses")) %>% kable_styling()
```
...
# Show kable of all (cleaned) responses with raw counts and percent
kbl_responses(friend_meetup, "friend_where_clean") %>% kable_styling()
...
```{r coins-visualize, echo=FALSE, message=FALSE, warning=FALSE}
...
<span style="color:green">Pick a number 0 to 10. You win if you match your partner.</span>
...
<span style="color:green">Select a number from the set. You win if you match your partner.</span>
...
<span style="color:green">Pick any number. You win if you match your partner, *and* the bigger the number, the more you both win.</span>
